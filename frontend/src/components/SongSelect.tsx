...
import { fetchSongList, uploadMusic, regenerateChart } from '../utils/api';
...
const SongSelect: React.FC = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { song_id } = await uploadMusic(file, nameToUse);
    setUploading(false);
    setUploadName('');
    e.target.value = '';
    await loadSongs();
  ...
  const handleRegen = async () => {
    ...
    await regenerateChart(targetSong.song_id, regenPrompt, regenKey);
        ...
        {modalVisible && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-4 rounded shadow w-80">
              <h3 className="text-lg font-semibold mb-2">Regenerate Chart</h3>
              <textarea
                value={regenPrompt}
              ...
              <div className="flex justify-center space-x-2 mb-2">
                {[4,5,6].map(k => (
                  <button
                    ...
                    className={`px-3 py-1 rounded ${regenKey === k ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
                  >
                    {k}-Keys
                  </button>
                ))}
